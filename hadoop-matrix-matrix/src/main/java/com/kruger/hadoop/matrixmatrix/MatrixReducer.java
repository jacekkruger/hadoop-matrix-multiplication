...
public class MatrixReducer extends Reducer<Text, Text, Text, Text> {
	...
	protected void reduce(Text keyin, Iterable<Text> values, Context context) throws IOException, InterruptedException {
		...
		Map<BigInteger, BigInteger> leftElements = sides.getOrDefault(MatrixSide.LEFT, emptyList()).stream()
				.collect(groupingBy(IntermediateElement::getNumber,
						mapping(IntermediateElement::getValue, singletonCollector())));

		Map<BigInteger, BigInteger> rightElements = sides.getOrDefault(MatrixSide.RIGHT, emptyList()).stream()
				.collect(groupingBy(IntermediateElement::getNumber,
						mapping(IntermediateElement::getValue, singletonCollector())));

		BigInteger sum = BigInteger.ZERO;
		for (Entry<BigInteger, BigInteger> l : leftElements.entrySet()) {
			for (Entry<BigInteger, BigInteger> r : rightElements.entrySet()) {
				if (l.getKey().equals(r.getKey())) {
					sum = sum.add(l.getValue().multiply(r.getValue()));
				}
			}
		}

		log.info("Emmiting (" + keyin + ") -> (" + sum + ")");
		context.write(keyin, new Text(sum.toString()));
